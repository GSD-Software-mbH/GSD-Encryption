...
class EncryptionManager {
  ...
  Future<String> encryptAES(String plainText,
      {Key? key, String? padding = "PKCS7"}) async {
    ...
    final encrypter = Encrypter(AES(key!,
        mode: AESMode.cbc, padding: padding)); // Verwendet AES im CBC-Modus
  ...
  Future<String> decryptAES(String encryptedText,
      {Key? key, String? padding = "PKCS7"}) async {
    // Initialisiert den AES-Schlüssel, falls nicht vorhanden
    if (key == null) await initializeAESKey();
  ...
  Future<Uint8List> decryptRSA(Uint8List encryptedBytes,
    ...
    final decryptedBytes = Uint8List.fromList(
        rsaEncrypter.decryptBytes(Encrypted(encryptedBytes)));
  ...
  Future<Uint8List> encryptRSA(Uint8List plainBytes,
  ...
  ///
  /// Wirft eine Exception, wenn kein Schlüssel verfügbar ist oder die Verschlüsselung fehlschlägt.
  Future<String> encryptRSAInBlocks(String plainText,
      {RSAPublicKey? publicKey}) async {
    // Initialisiert das RSA-Schlüsselpaar, falls nicht vorhanden
    if (publicKey == null) await initializeRSAKeyPair();
  ...
  Future<AsymmetricKeyPair<RSAPublicKey, RSAPrivateKey>> generateRandomRSAKey(
      {int bitLength = 2048}) async {
    RSAPublicKey publicKey;
    RSAPrivateKey privateKey;

    if (foundation.kIsWeb) {
      return await WebRSAEncryptionManager.generateRSAKeys(
          bitLength: bitLength);
    }
