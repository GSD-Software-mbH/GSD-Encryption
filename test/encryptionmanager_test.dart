...
void main() {
  ...
  setUp(() {
    ...
    final keyBytes = List<int>.generate(
        32, (_) => secureRandom.nextInt(256)); // 256-Bit-Schlüssel für AES
  ...
  group('AES Encryption and Decryption', () {
    test('Encrypt and decrypt AES should return original text', () async {
      ...
      final encryptedText =
          await encryptionManager.encryptAES(plainText, key: aesKey);
      final decryptedText =
          await encryptionManager.decryptAES(encryptedText, key: aesKey);
  ...
  group('RSA Encryption and Decryption', () {
    test('Encrypt and decrypt RSA should return original text', () async {
      ...
      final encryptedText =
          await encryptionManager.encryptRSAInBlocks(plainText);
      final decryptedText =
          await encryptionManager.decryptRSAInBlocks(encryptedText);

      expect(decryptedText, equals(plainText));
    });

    test(
        'Encrypt and decrypt RSA should return original text (process in blocks)',
        () async {
      const plainText = 'Hello RSA Encryption';

      await encryptionManager.initializeRSAKeyPair();
      final encryptedText = await encryptionManager
          .encryptRSA(Uint8List.fromList(utf8.encode(plainText)));
      final decryptedText =
          utf8.decode(await encryptionManager.decryptRSA(encryptedText));
  ...
  group('RSA Key initialization from Filepaths', () {
    test('RSA key pair should be initialized from Filepaths', () async {
      EncryptionManager privateEncryptionManager = EncryptionManager.init(
          EncryptionOptions(
              rsaPrivateKeyFilePath: "cert\\encryption.pem",
              rsaPublicKeyFilePath: "cert\\encryption-pub.pem"));
