...
class WebRSAEncryptionManager {
  ...
  static Future<String> _exportPublicKey(CryptoKey publicKey) async {
    ...
    final spkiKey =
        await window.crypto.subtle.exportKey('spki', publicKey).toDart;

    final spkiKeyBuffer = (spkiKey as JSArrayBuffer)
        .toDart; // Verwende toDart für die Konvertierung
    final spkiKeyBytes =
  ...
  static Future<String> _exportPrivateKey(CryptoKey privateKey) async {
    ...
    final pkcs8Key =
        await window.crypto.subtle.exportKey('pkcs8', privateKey).toDart;

    final pkcs8KeyBuffer = (pkcs8Key as JSArrayBuffer)
        .toDart; // Verwende toDart für die Konvertierung
    final pkcs8KeyBytes =
